...
void main() {
  List<List<TerrainCell>> _makeGrid({CellType fill = CellType.rock}) {
    ...
    return List.generate(
      ...
      (y) => List.generate(
        size,
        (x) => TerrainCell(type: fill, density: fill == CellType.empty ? 0.0 : 0.8),
      ),
    );
  }
  ...
  test('1. Chunk survives save/load round trip', () {
    ...
    final restored = serialized
        .map((row) =>
            row.map((map) => TerrainCell.fromMap(map)).toList())
        .toList();
  ...
  test('4. Border cells aware of adjacent chunks', () {
    ...
    final chunk01 = gen.generateChunk(0, 1);
    final chunk11 = gen.generateChunk(1, 1);
  ...
  test('5. Empty vs full chunk data comparison', () {
    final emptyGrid = _makeGrid(fill: CellType.empty);
    final fullGrid = _makeGrid(fill: CellType.rock);
